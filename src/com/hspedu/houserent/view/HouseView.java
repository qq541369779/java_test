...
public class HouseView {
    ...
    public void update(){
        ...
        String phone = Utility.readString(12,""); //如果用户直接回车表示不修改该信息，默认""
        if(!"".equals(phone)){ // 修改
            house.setPhone(phone);
        }

        System.out.println("地址("+house.getAddress()+")：");
        String address = Utility.readString(18,""); //如果用户直接回车表示不修改该信息，默认""
        if(!"".equals(address)){ // 修改
            house.setAddress(address);
        }

        System.out.println("租金("+house.getRent()+")：");
        int rent = Utility.readInt(-1); //如果用户直接回车表示不修改该信息，默认""
        if(rent != -1){ // 修改
            house.setRent(rent);
        }

        System.out.println("状态("+house.getState()+")：");
        String state = Utility.readString(3,""); //如果用户直接回车表示不修改该信息，默认""
        if(!"".equals(state)){ // 修改
    ...
    public void delHouse() {
        ...
        if (delId == -1) {
            System.out.println("==========放弃删除房屋息==========");
            return;
        }
        // 注意该方法本身就有循环判断的逻辑，必须输出Y/N，否则出不来
